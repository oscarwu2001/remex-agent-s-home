...
| **Room item** | Anything standing in a room, placed on its 6 × 6 tile grid: the furniture every room starts with, decorations and fun extras. Any item may go in any room (`suits` only orders the list); doorways stay clear. Decoration spots of earlier versions become items on the grid. |
...
| **Meadow** | The second screen: a terraced block island (3 × 3 plots at different heights) where every agent lives as a blocky isometric creature (10 kinds × 3 stages × 10 looks) that wanders the tiles. Its residents are the installed agents, every agent called in the transcripts the report reads (`seen` from `--stats-only`, the last 56 days), anyone at work now, and everyone met before (kept in the preferences as `meadowSeen`). Creatures are sized by their stage and running token total, and show the report's per-agent numbers. Looks are kept per viewer in the app's preferences. |
| **Egg / hatch / evolve** | A Meadow creature's life (`renderer/growth.js`): an egg until its agent has used 100k tokens, then a starter picked from three, then three stages that evolve at 1M and 5M tokens. The tokens are a running total kept per agent per day in the preferences (`ledger`, merged from the stats' `tokensByDay`). What each agent hatched into is `nest`. |
