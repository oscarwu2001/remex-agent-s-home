...
const config = () => ({
  ...
  decorCatalogue: {
    ...
    gardenSize: D.GARDEN_SIZE, gardens: D.GARDENS, plants: D.PLANTS, defaultGardens: D.DEFAULT_GARDENS,
  },
  ...
  .createServer((req, res) => {
    ...
    if (url.pathname === '/preview/layout' && req.method === 'POST') {
      ...
      req.on('end', () => {
        ...
        try {
          ...
          decor = { ...decor, rooms: Object.fromEntries(Object.entries(decor.rooms).filter(([id]) => ids.has(id))) };
    ...
    if (url.pathname === '/preview/decor' && req.method === 'POST') {
      ...
      req.on('end', () => {
        let out;
        try {
          const { decor: next, problems } = D.checkDecor(JSON.parse(body), roomsWith(layout));
